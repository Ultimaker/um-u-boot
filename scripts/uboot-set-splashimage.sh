...
if [ "${eeprom_art_num}" = "${S3_ARTNUM}" ]; then
    bunzip2 -c  "${script_dir}/${S3SPLASH}.bmp.bz2" > "/boot/${SPLASHFILE}.bmp"
elif [ "${eeprom_art_num}" = "${S5_ARTNUM}" ]; then
    bunzip2 -c  "${script_dir}/${S5SPLASH}.bmp.bz2" > "/boot/${SPLASHFILE}.bmp"
else
    bunzip2 -c  "${script_dir}/${UMSPLASH}.bmp.bz2" > "/boot/${SPLASHFILE}.bmp"
fi

# Store the EEPROM Bom number in a cache file for next boots.
